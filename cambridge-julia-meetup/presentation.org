...
\end{equation*}
Therefore if we can compute $\mathcal{J}_{b^{-1}}(y)$ we have an analytical expression for $\tilde{p}(y)$!
...
#+begin_quote
$b_{\phi}$ is often referred to as *normalising flow (NF)*
#+end_quote
...
#+begin_src jupyter-julia :session jl :exports code :file figures/nf-mle-estimate.svg
